...
class EDDError(Exception):
    """
    Parent Warning for "plain" errors.

    Contains no boilerplate meant for reading by end-users.
    """


class EDDWarning(Warning):
    """
    Parent Warning for "plain" warnings.
...
class ReportingLimitWarning(EDDWarning):
    pass
...
class EDDReportableError(MessagingMixin, EDDError):
...
class EDDReportableWarning(MessagingMixin, EDDWarning):
    def __init__(self, **kwargs):
        if "category" not in kwargs:
